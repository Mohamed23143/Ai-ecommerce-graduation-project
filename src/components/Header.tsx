...
const Header = ({ backLabel = 'Back to Collection', backTo = '/' }: HeaderProps) => {
  const [mobileOpen, setMobileOpen] = useState(false);
  const { cartCount, setIsCartOpen, setIsSearchOpen } = useCart();

  return (
    <>
      <header id="main-header" className="sticky top-0 z-[100] bg-[#f9f8f5]/95 backdrop-blur-md border-b border-border-light/50">
        <div className="max-w-8xl mx-auto px-5 lg:px-12 h-[64px] flex items-center justify-between">
          {/* Left: Back Link */}
          <Link
            to={backTo}
            id="back-to-collection"
            className="hidden md:flex items-center gap-2 text-[11px] font-sans tracking-widest-xl uppercase text-dark hover:text-gold transition-colors duration-300 group"
          >
          ...
          <button
            ...
            className="md:hidden p-1 text-dark hover:text-gold transition-colors cursor-pointer"
          ...
          <Link to="/" className="absolute left-1/2 -translate-x-1/2">
            <h1 className="font-sans text-[22px] tracking-[0.35em] uppercase font-medium text-dark">
              NASSEG
            </h1>
          </Link>
