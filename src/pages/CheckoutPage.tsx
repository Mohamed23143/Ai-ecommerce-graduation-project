...
const CheckoutPage = () => {
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Clerk tokens expire quickly (~60s), so we must fetch at submission time
      ...
      // If token expired, try once more with a completely fresh token
      if (err.message?.includes('expired') || err.message?.includes('Token') || err.message?.includes('401')) {
        try {
          const freshToken = await getToken({ skipCache: true });
          if (!freshToken) throw new Error('Authentication required. Please sign in again.');
          const result = await placeOrder({
            email,
            items: items.map((i) => ({ product_id: i.id, quantity: i.quantity })),
          }, freshToken);
          ...
          setOrderError(retryErr.message || 'Failed to place order. Please try again.');
          return;
        }
      }
    ...
    return (
      <div className="min-h-screen bg-dark flex flex-col items-center justify-center p-5 text-white">
        ...
        <div className="space-y-4 w-full max-w-xs">
          ...
          <Link to="/account" className="block w-full text-center border border-white/20 text-white/60 py-4 text-[11px] font-sans tracking-widest-2xl uppercase hover:border-white hover:text-white transition-all">Track Your Order</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#f9f8f5] flex flex-col lg:flex-row overflow-hidden">
      ...
      <ScrollReveal variant="fade-up" className="lg:w-[60%] p-5 lg:p-12 flex flex-col">
              ...
              {step >= 2 && (
                <div className="space-y-4">
                  <label className={`flex items-center justify-between p-5 border cursor-pointer transition-all ${shippingMethod === 'standard' ? 'border-dark bg-white' : 'border-border-light bg-transparent hover:border-dark/30'}`}>
                    ...
                    <span className="text-sm font-sans text-dark">{formatPrice(15)}</span>
                  ...
                  <label className={`flex items-center justify-between p-5 border cursor-pointer transition-all ${shippingMethod === 'express' ? 'border-dark bg-white' : 'border-border-light bg-transparent hover:border-dark/30'}`}>
                    ...
                    <span className="text-sm font-sans text-dark">{formatPrice(25)}</span>
      ...
      <ScrollReveal variant="fade-left" className="lg:w-[40%] bg-white lg:bg-dark p-8 lg:p-12 flex flex-col min-h-screen">
